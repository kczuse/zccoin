...
static CBigNum bnInitialHashTarget(~uint256(0) >> 25);
...
const string strMessageMagic = "ZcCoin Signed Message:\n";
...
int64 GetProofOfWorkReward(unsigned int nBits)
{
    ...
    CBigNum bnTargetLimit = bnInitialHashTarget;
    bnTargetLimit.SetCompact(bnTargetLimit.GetCompact());

    // ZcCoin: subsidy is cut in half every 64x multiply of PoW difficulty
    // A reasonably continuous curve is used to avoid shock to market
    // (nSubsidyLimit / nSubsidy) ** 6 == bnProofOfWorkLimit / bnTarget
    //
    // Human readable form:
    //
    // nSubsidy = 50 / (diff ^ 1/6)
...
static const int64 nTargetSpacingWorkMax = 12 * nStakeTargetSpacing; // 12 min
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in ZcCoin
    bool fStrictPayToScriptHash = true; // Always active in ZcCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "ZcCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Search for Mr Rich goes awry. China Daily 16:31, August 06, 2013. There is one more thing, Geng, in the presence of ZcCoin, take you to be my wife; to have and to hold from this day forward, for better, for worse, for richer, for poorer, in sickness and in health, to love and to cherish, so long as we both shall live. All this I vow and promise.";
        CTransaction txNew;
...
unsigned char pchMessageStart[4] = { 0x20, 0xe3, 0x35, 0xe0 };
